...
const states = [
  { name: "registration", query: "state=registration", marker: "Register now" },
  { name: "public", query: "state=public", marker: "Sign in" },
  { name: "registered", query: "state=registered", marker: "CGSC-7KQ9M2RX" },
  { name: "producer", query: "state=producer", marker: "Writer account required" },
  { name: "live", query: "state=live", marker: "See the theme" },
  { name: "results", query: "state=results", marker: "Last Stop" },
  { name: "entry-error", query: "state=entry-error", marker: "Entry check offline" },
  { name: "error", query: "state=error", marker: "Challenge service offline" },
  { name: "missing", query: "state=missing", marker: "Challenge not found" },
];
...
for (const state of states) {
  for (const width of widths) {
    ...
    await cdp.call("Page.navigate", { url: `${APP_ORIGIN}/__mobile-challenge-detail?${state.query}` });
    let shellReady = false;
    for (let attempt = 0; attempt < 200; attempt += 1) {
      const ready = await cdp.call("Runtime.evaluate", {
        expression: "Boolean(document.querySelector('.ckm-challenge-detail') && document.querySelector('[data-shell-mode=\"detail\"]'))",
        returnByValue: true,
      });
    ...
    if (!shellReady) failures.push(`${state.name}@${width}: detail shell did not become ready`);
    ...
    const checks = [
      [value.shell === "detail", `shell ${value.shell} != detail`],
...
if (failures.length) {
  ...
  console.log(`Challenge detail sweep passed: ${results.length} state-width measurements, 0 findings.`);
}
