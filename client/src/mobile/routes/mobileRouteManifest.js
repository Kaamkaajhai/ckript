...
export const MOBILE_ROUTE_DISPOSITIONS = Object.freeze([
  ...
  migration("accept-invite", "/invite/:token"),
  {
    ...
    reason: "The signed-out public profile uses the sanitized public projection (D34); authenticated visitors use the relationship-aware profile while an account's own workspace remains pending (D35).",
    protection: "public",
    visitorOnly: true,
  ...
  migration("messages", "/messages"),
  {
    ...
    pattern: "/profile/:id?",
    disposition: MOBILE_ROUTE_DISPOSITION.SCREEN,
    reason: "Authenticated visitor profile with follow, message, block, contact and pitch actions. The id-less/own variant remains on desktop until editing and settings are ported (plan §11 Phase 5, D35).",
    protection: "authenticated",
    visitorOnly: true,
  ...
  },
  {
    ...
    pattern: "/:id",
    disposition: MOBILE_ROUTE_DISPOSITION.SCREEN,
    reason: "The authenticated canonical username form of the D35 visitor profile. Signed-out referral/profile branching remains owned by App.jsx.",
    protection: "authenticated",
    visitorOnly: true,
    screenId: "profile-visitor",
    shell: MOBILE_SHELL_MODE.STANDARD,
