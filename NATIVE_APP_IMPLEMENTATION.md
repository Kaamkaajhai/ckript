...
| `/collaborations` | `pages/Collaborations.jsx` + `components/collab/` | `collaboration/CollaborationRequestsMobile.jsx` | **DONE — PROMOTED TO writer `SCREEN` 2026-08-21 (D43), REQUEST SLICE.** URL-backed Incoming/Sent tabs, bounded server paging, current role choices, owner decisions, actionable invite notifications, and shared desktop/native request/status/activity operations. People, comments and presence remain in the native editor; the visible activity/invitation-lifecycle finish is D44. |
...
- [ ] Collaboration requests, invites, presence, comments, and activity. *(D43 completes incoming/sent request state, owner decisions, writer-role authorization, paging, notification targets, and the canonical `/collaborations` mobile entry. People/comments/presence were already native in Phase 3. A visible native activity view and complete invitation lifecycle remain for D44.)*
...
latest_completed_work_item: "Native collaboration request queue and shared paged decision contract (2026-08-21, tenth Phase 5 session, D43)."
current_phase: 5
current_work_item: "COMPLETE — Codex — 2026-08-21. D43: `/collaborations` promoted with shared incoming/outgoing/status/decision/activity operations and corrected invite/request notification targets."
...
next_action: "BEGIN D44 at `components/collab/InviteModal.jsx`, `components/collab/ActivityLog.jsx`, `components/collab/useCollaborators.js`, `pages/AcceptInvite.jsx`, and `mobile/screens/collaboration/CollaborationRequestsMobile.jsx`: finish the collaboration family with a visible native paged activity view and an invitation lifecycle that covers pending account and email-only invites, refresh/expiry, in-app acceptance, and durable success/failure without duplicating the editor's already-native People/comments/presence surfaces. Reuse the D18 collaborator hook and D43 request/activity contract. Keep banking/guild-proof review separate."
open_follow_ups:
...
updated_by: "Codex D43 native collaboration requests and shared paged decision contract"
```
...
| 5. Profiles/network/messages | IN PROGRESS | Codex | 2026-08-21 | — | **D33–D43 COMPLETE:** signed-out public, authenticated visitor, own-profile overview/editor/settings, inbound follow requests, full message inbox/thread including attachments/mutations/context/meetings, own/visitor reader profile, and the incoming/sent collaboration-request queue are native. Shared contracts own profile/actions/security, follow decisions, message requests/mutations/context, meetings, reader activity, and collaboration request/status/decision/activity operations. Banking/guild proof, general activity/saved lists, and the visible invitation/activity collaboration finish remain. |
