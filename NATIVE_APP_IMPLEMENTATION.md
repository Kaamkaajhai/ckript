...
| Create project | `ckm-create-project` (the wizard chrome: step position, panels, sticky footer) |
...
| `/create-project`, `/create-project/:draftId` | `pages/CreateProject` | **mode A:** `mobile/screens/create/Editor.jsx` (`ckm-editor`, `immersive` + `EDITOR_SHELL_SLOTS`) — **BUILT 2026-08-09**. **mode B:** the publish wizard (steps 2–5) — NOT STARTED | **STILL `DESKTOP_MIGRATION_FALLBACK`.** The editor is built, tested (43 tests) and swept at five widths, but the route carries both modes and promoting it with an unported wizard would put desktop form markup on the phone the moment a writer taps "Continue to details" (§2.2). Reachable meanwhile at the development route `/__mobile-editor`. Promotion lands with mode B |
...
current_phase: 3
current_work_item: "PHASE 3 IS OPEN. Bullet 1 (research spike) COMPLETE 2026-08-08. Bullet 2 PARTIAL: the save/resume core landed for BOTH platforms (DEF-1, DEF-2 fixed), /new-project shipped, and as of 2026-08-09 (later) the /create-project EDITOR (mode A) is built, tested and swept — but the route is STILL a DESKTOP_MIGRATION_FALLBACK, because mode B (the publish wizard, steps 2-5) is not built and a route cannot be half-promoted. Bullet 4 is PARTIAL for the same delivery. Bullets 3, 5, 6 NOT STARTED."
previous_work_item: "Phase 3 bullet 2, first half (2026-08-09) — the save/resume core (lib/workingDraft.js, lib/keepaliveSave.js, both platforms) and the mobile /new-project chooser."
last_completed_work_item: "Phase 3, the screenplay editor surface / mode A (2026-08-09, later). New: mobile/screens/create/{editorChrome.js, EditorDock.jsx, Editor.jsx, Editor.css, EditorDock.render.test.jsx, Editor.render.test.jsx} and mobile/dev/EditorHarness.jsx. Changed: mobile/shell/mobileShellModes.js (resolveShellSlots / changedShellSlots / assertShellSlotOverride / MOBILE_SHELL_SLOTS + new mobileShellModes.test.js), mobile/shell/MobileShell.jsx (`slots` prop, data-shell-slots), mobile/shell/MobileShell.css (immersive keeps its chrome slots transparent and un-padded), mobile/theme/cssPrefixRegistry.js (ckm-editor registered), mobile/routes/{MobileRoutes.jsx, mobileRouteManifest.js, mobileRouteCoverage.test.js} (the /__mobile-editor dev route), src/App.jsx (same). SHARED, both platforms: components/screenplay/screenplayMode.js — Escape now blurs the editor, fixing DEF-6, a live WCAG 2.1.2 keyboard trap. NOTE: the route manifest entries for /create-project and /create-project/:draftId were NOT changed."
next_action: "Build MODE B — the mobile publish wizard, /create-project steps 2-5 — and promote /create-project and /create-project/:draftId to SCREEN in the SAME change. Mode A (the editor) is done: mobile/screens/create/Editor.jsx, verified at five widths, reachable at the dev route /__mobile-editor. What mode B needs: (a) the §4.3 wireframe for it is already written (2026-08-08 §19.3, 'mode B — details → publish') — flow shell, app bar 'Step N of 5 · <name>', ONE panel at a time, sticky Back/Next footer above the keyboard, Next becomes Publish on step 5; (b) port Step2Details (693 lines, a mini-wizard of up to six sub-panels — the biggest single item), Step3Classify (60), Step4FilmInfo (90) and Step5Publish (243) as ckm-create-project screens, reusing the existing ckm-field/ckm-control form family rather than the desktop Tailwind markup (§2.2); (c) resume must land on the exact step AND Details sub-panel the writer left — lib/workingDraft.js already records both (D7); (d) THE SEAM: pages/CreateProject/index.jsx currently hardcodes <CreateProjectShell>. Give it an injectable chrome (a `Shell` component plus flags suppressing the desktop exit-confirm and drafts drawer when a native chrome owns them, defaulted so desktop is byte-identical), then mobile renders the orchestrator with mobile chrome. This was deliberately NOT written this session — its shape depends on the wizard it has to serve. (e) Retire /__mobile-editor when the route is promoted. Independent and safe at any point, carried forward: (i) DEF-3, the corkboard reorder, still a live desktop WCAG 2.1.1 failure and cheap (moveScene is already pure); (ii) migrate ckm-tabs (components/SectionTabs) onto ckm-tabbar, the last legacy prefix with a caller; (iii) add the dashboard socket refresh — desktop refetches on seven collab_* events, mobile still never refreshes after first load."
open_follow_ups:
...
updated_by: "Claude Phase 3 session (the screenplay editor surface, mode A; the shell slot-override mechanism; DEF-6 fixed)"
```
...
#### 2026-08-09 (later, latest) — Claude (Claude Code) — Phase 3, the screenplay editor surface (mode A)
