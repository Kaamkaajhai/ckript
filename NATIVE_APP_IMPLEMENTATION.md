...
latest_completed_work_item: "Native incoming follow-request queue and shared network decision contract (2026-08-21, fifth Phase 5 session, D38)."
current_phase: 5
current_work_item: "COMPLETE — Codex — 2026-08-21. D38: `/follow-requests` is native, owner-linked, and backed by one cancellable list/decision state shared with desktop and notification shells; malformed requester ids and stale accepted-request notifications are fixed server-side."
...
next_action: "BEGIN D39: run the Phase 5 research gate across `/messages`, message routes/controllers, socket events, unread counters, composer, attachments, and request/approval states; document the thread-list/detail state machine first, then extract the smallest shared conversation load/send boundary before promoting any route. Keep reader collections and bank/guild-proof review separate."
open_follow_ups:
...
| 5. Profiles/network/messages | IN PROGRESS | Codex | 2026-08-21 | — | **D33–D38 COMPLETE:** signed-out public, authenticated visitor, own-profile overview/editor/settings, and inbound follow-request surfaces are native. Shared contracts own profile loading/actions, identity/professional mutation, account security, and incoming list/accept/reject state across desktop, mobile, and both notification shells. `GET /users/:id` applies explicit visitor allowlists and owner secret redaction. Banking/guild proof, reader profiles, messages, meetings and collaboration remain. |
