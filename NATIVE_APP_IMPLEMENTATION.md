...
next_action: "BEGIN D30: `/script/:id/pay`, the buyer's checkout, and then the public shared project. Payment is the one Phase 4 surface with a third-party overlay OUTSIDE our DOM — Razorpay Checkout — so the native rules that matter are different from every screen so far: back during checkout must not orphan a charged payment, and the five states (already purchased, not approved for payment, free, gateway blocked, verification failed AFTER the charge) can only be told apart against the detail screen D29 just finished. Read `useProjectActions` first: the approved-request standing it renders is the state the pay route begins from. DEF-27 (`GET /scripts/:id/pdf` serving the complete screenplay to anyone past the marketplace gate) is STILL LIVE and still blocked on the product decision recorded below — it is the most serious open item in this plan and it belongs to this family."
open_follow_ups:
