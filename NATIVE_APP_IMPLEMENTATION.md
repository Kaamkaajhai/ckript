...
| `/create-project`, `/create-project/:draftId` | `pages/CreateProject` | `mobile/screens/create/CreateProjectRoute.jsx` → `CreateProjectChrome` → **mode A** `Editor.jsx` (`ckm-editor`, `immersive` + `EDITOR_SHELL_SLOTS`) when `step === 1`, **mode B** `Wizard.jsx` (`ckm-create-project`, `flow` + `WIZARD_SHELL_SLOTS`) for steps 2–5 | **DONE — PROMOTED TO `SCREEN` 2026-08-09; MEDIA RECOVERY ADDED 2026-08-12 (D21); CANCELLATION/PREFLIGHT ADDED 2026-08-13 (D22).** Both modes built, 10 panels ported onto the shared families, editor tools complete through Reports, and create-project media now shows real byte progress, deterministic large-file preflight, explicit cancellation, and failed/cancelled-file retry without duplicate project creation. The orchestrator is shared, not forked: `<CreateProject Shell={CreateProjectChrome} nativeChrome hostClassName=… />`. **ONE DECLARED EXCLUSION:** `?ctx=competition` stays on desktop (`excludeQuery` in the manifest) — competition mode replaces the whole wizard with `CompetitionBar` + `CompetitionPitch` and a one-way Submit, neither ported, and shipping without the exclusion would leave a competition writer with no way to submit at all |
...
| Creation/editor | editor toolbar, title page, AI tools, collaboration, presence, version history, reports, corkboard | mobile editor research spike; progressive/immersive UI | **PARTIAL — BULLET 4 COMPLETE (2026-08-12); CLIENT MEDIA SAFETY COMPLETE (2026-08-13, D22).** Built: docked Elements/Format toolbar, overflow/export/exit flows, title page, cover cropper, buyer preview, comments, live people/access administration, version history/diff/restore, Navigator, corkboard with non-drag reorder, scene/character Reports with shared PDF/CSV export, AI quota states, real media progress, deterministic large-file preflight, explicit batch cancellation, and honest whole-file recovery. The family remains PARTIAL for the server-backed resumable-media contract, the declared competition exclusion/product decisions, and the editor's recorded real-device keyboard/screen-reader verification debts. |
...
- [~] File/image/video picker and interrupted-upload recovery. *(**Client safety/recovery delivered across both creation routes; `/create-project` parity added 2026-08-12 (D21), cancellation/preflight added 2026-08-13 (D22).** The pickers are the shared `ckm-media` `MediaSlot` (a real `<input type="file">` behind a `<label>`, per MDN — drag-and-drop is not ported because a touch screen has nothing to drag from). Both creation entry points use `lib/projectMediaUpload.js` for real axios byte progress, one fresh AbortController per concurrent batch, cancelled-versus-failed results, and a deterministic 25 MiB warning for trailers/pitch videos before the base write starts. Completed files remain complete; unfinished requests become **Upload cancelled**, never **Upload failed**. Both flows retain the target script id, retry only unfinished media, and never recreate the project. Every retry resets to 0%; it is explicitly not resume. Still open: implementation of D21's separate server-backed resumable contract.)*
...
latest_completed_work_item: "Native media cancellation and deterministic large-file preflight (2026-08-13, sixteenth Phase 3 session, D22)."
current_phase: 3
current_work_item: "COMPLETE 2026-08-13 (sixteenth Phase 3 session) — MEDIA CANCEL + PREFLIGHT (D22). `projectMediaUpload.js` owns one AbortSignal/result contract and one size-only preflight model; both orchestrators create a fresh controller per attempt; both native media panels render confirm, active/cancel, and cancelled/retry states; user cancellation is proven distinct from genuine failure."
previous_work_item: "Create-project media upload progress and whole-file recovery (2026-08-12, fifteenth Phase 3 session, D21)."
...
next_action: "PRODUCT DIRECTION REQUIRED BEFORE PHASE 3 CLOSE: decide whether to port the declared `?ctx=competition` exclusion and whether the writer mobile bottom bar should restore Projects or formally keep Create. D21's server-backed resumable-media contract remains separate backend work and must not be represented as client resume."
open_follow_ups:
  ...
  - "D9'S CLIENT POLICY IS FIXED (2026-08-13, D22). Both native creation routes warn deterministically when a selected trailer or pitch is at least 25 MiB, before any base record is written; exact files and total bytes are listed. One fresh AbortController cancels the current concurrent batch, completed files remain done, unfinished files become `cancelled` rather than `failed`, and retry starts from 0%. `navigator.connection` is deliberately unused because availability is not dependable. The only remaining media-recovery gap is D21's separate server-backed resumable contract."
...
updated_by: "Codex Phase 3 sixteenth-session completion (media cancellation + preflight, D22)"
```
