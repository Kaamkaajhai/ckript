...
| `/messages` | `features/messages-operator` | `messages/MessagesMobile.jsx` | **DONE — PROMOTED TO `SCREEN` 2026-08-21 (D39/D40).** URL-owned inbox/detail, pending-recipient deep links, authoritative unread state, text and attachment-only sends, upload selection/progress/retry/removal, attachment rendering, touch reactions, confirmed own-message deletion, authenticated sockets and visible-tab polling. Desktop and mobile share `messageContract.js`; upload grants bind every message asset to its authenticated sender and intended recipient before multipart parsing. Meeting/deal context remains a separate slice. |
...
latest_completed_work_item: "Native message attachments, reactions, confirmed deletion, and scoped mutation authority (2026-08-21, seventh Phase 5 session, D40)."
current_phase: 5
current_work_item: "COMPLETE — Codex — 2026-08-21. D40: attachments, reactions, and own-message deletion are native and shared; uploads are recipient-authorized before parsing and server grants plus authoritative socket events replace client trust."
...
next_action: "BEGIN D41 in `features/messages-operator/` and `mobile/screens/messages/`: audit the desktop meeting launcher, Google Calendar connect/return path, script/deal-context sidebar, and message-origin deep links; reuse the existing project-detail meeting contract where its authorization matches, then add a native message-thread meeting/context surface without copying `MeetingModal`. Keep reader collections and bank/guild-proof review separate."
open_follow_ups:
...
updated_by: "Codex D40 native message mutations"
```
...
| 5. Profiles/network/messages | IN PROGRESS | Codex | 2026-08-21 | — | **D33–D40 COMPLETE:** signed-out public, authenticated visitor, own-profile overview/editor/settings, inbound follow requests, and the complete core message inbox/thread are native. Shared contracts own profile loading/actions, identity/professional mutation, account security, incoming list/accept/reject, conversation requests, attachment upload, reactions, and own-message deletion. `GET /users/:id` applies explicit allowlists/redaction; message unread aggregation, upload grants, participant authorization, and server-authoritative socket delivery are fixed server-side. Banking/guild proof, reader profiles/collections, message-thread meeting/deal context, and collaboration remain. |
