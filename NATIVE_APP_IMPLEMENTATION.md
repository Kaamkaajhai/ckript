...
latest_completed_work_item: "Native public shared project at /share/project/:id (2026-08-20, seventh Phase 4 session, D31)."
current_phase: 4
current_work_item: "COMPLETE — Codex — 2026-08-20. D31 promoted `/share/project/:id` from migration fallback to SHARED_PUBLIC_SCREEN. New: `pages/script-detail/usePublicProject.js` and `mobile/screens/projects/public-project/` (`ProjectPublicMobile.{jsx,css}`, `publicProjectModel.js` and focused model tests). Changed: MobileRoutes, the manifest, route-policy coverage, and the prefix registry (`ckm-public-project`)."
...
next_action: "BEGIN D32: Phase 4's remaining reader/preview modes and long-content performance. Audit `/reader/script/:id`, `features/script-workbench/ScriptWorkbenchPage.jsx`, `ScreenplayPdfViewer`, and the authenticated `ProjectReaderDialog` together. Resolve the still-open DEF-27 product choice before exposing uploaded-PDF previews: the current `/scripts/:id/pdf` endpoint returns the complete file to preview-entitled authenticated viewers, so a native reader must not reproduce client-side page limiting as access control."
open_follow_ups:
