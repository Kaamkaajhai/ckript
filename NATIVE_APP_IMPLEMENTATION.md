...
| `/create-project`, `/create-project/:draftId` | `pages/CreateProject` | `mobile/screens/create/CreateProjectRoute.jsx` → `CreateProjectChrome` → **mode A** `Editor.jsx` (`ckm-editor`, `immersive` + `EDITOR_SHELL_SLOTS`) when `step === 1`, **mode B** `Wizard.jsx` (`ckm-create-project`, `flow` + `WIZARD_SHELL_SLOTS`) for steps 2–5 | **DONE — PROMOTED TO `SCREEN` 2026-08-09; MEDIA RECOVERY ADDED 2026-08-12 (D21).** Both modes built, 10 panels ported onto the shared families, editor tools complete through Reports, and create-project media now shows real byte progress plus failed-file retry without duplicate project creation. The orchestrator is shared, not forked: `<CreateProject Shell={CreateProjectChrome} nativeChrome hostClassName=… />`. **ONE DECLARED EXCLUSION:** `?ctx=competition` stays on desktop (`excludeQuery` in the manifest) — competition mode replaces the whole wizard with `CompetitionBar` + `CompetitionPitch` and a one-way Submit, neither ported, and shipping without the exclusion would leave a competition writer with no way to submit at all |
...
| Creation/editor | editor toolbar, title page, AI tools, collaboration, presence, version history, reports, corkboard | mobile editor research spike; progressive/immersive UI | **PARTIAL — BULLET 4 COMPLETE (2026-08-12).** Built: docked Elements/Format toolbar, overflow/export/exit flows, title page, cover cropper, buyer preview, comments, live people/access administration, version history/diff/restore, Navigator, corkboard with non-drag reorder, and scene/character Reports with shared PDF/CSV export. AI quota states are complete. The family remains PARTIAL only because Phase 3's media-progress/resumable-upload work and the editor's recorded real-device keyboard/screen-reader verification debts remain. |
...
- [~] File/image/video picker and interrupted-upload recovery. *(**Substantially delivered; `/create-project` parity added 2026-08-12 (D21).** The pickers are the shared `ckm-media` `MediaSlot` (a real `<input type="file">` behind a `<label>`, per MDN — drag-and-drop is not ported because a touch screen has nothing to drag from). Both creation entry points now use `lib/projectMediaUpload.js` for real axios byte progress. `/create-project` returns to the visible Visual assets panel only after its project record is saved, holds navigation while files send, marks each failure, and retains the target script id so **Retry the media upload** sends only failed files instead of posting the form again and creating a duplicate. A retry resets to 0%; it is explicitly not resume. Still open: D9's cancel action and deterministic large-file preflight warning, plus implementation of D21's server-backed resumable contract.)*
...
latest_completed_work_item: "Create-project media upload progress and whole-file recovery (2026-08-12, fifteenth Phase 3 session)."
current_phase: 3
current_work_item: "COMPLETE 2026-08-12 (fifteenth Phase 3 session) — CREATE-PROJECT MEDIA PROGRESS (D21). `projectMediaUpload.js` is now the one axios byte-progress path used by both `/create-project` and `/upload`. The create flow moves from Publish to the visible Media panel only after the project record exists; navigation is held while files send; a partial failure retains that script id and retries only failed types, preventing the former duplicate-project path. Retry starts at zero because this is whole-file retry, not resume."
previous_work_item: "Reports (2026-08-12, fourteenth Phase 3 session) — the final Phase 3 bullet 4 surface."
...
next_action: "PHASE 3 BULLET 6: add shared AbortController cancellation and a deterministic preflight warning for large trailer/pitch files to `pages/CreateProject/lib/projectMediaUpload.js`, surface both in the `/create-project` and `/upload` native media panels, and test cancel versus genuine failure. Do not call this resume: the server contract recorded in D21 must land first. Then Phase 3's exit gate still needs product direction on the declared `?ctx=competition` exclusion and the writer bottom-bar regression."
open_follow_ups:
  - "A 250 MB TRAILER RETRY IS STILL THE WHOLE FILE. Both `/upload` and `/create-project` now show real byte progress and a retry surface, but every retry begins at 0% and re-sends the file. D21 records the missing server contract: an authenticated, expiring upload session; idempotent chunk/part writes with checksums; a status endpoint returning accepted parts/ranges; atomic completion that attaches the final Cloudinary asset to the script; abort/expiry cleanup; and streaming or direct-to-cloud transfer instead of today's `multer.memoryStorage()` 250 MB process buffer. Until that exists, the client must never label retry as resume."
  - "D9'S CANCEL AND LARGE-FILE PREFLIGHT ARE STILL OPEN. There is no AbortController/signal in the shared uploader and no deterministic warning before a 250 MB trailer or 90 MB pitch starts. The next slice must add cancellation as a distinct state (not `failed`) and a size-based warning that does not rely on `navigator.connection`, whose availability is not dependable."
  ...
  - "THE DOCUMENTED RED BASELINE IS NO LONGER 2 TESTS — IT IS 13, AND THE NEW 11 ARE A PRODUCT REGRESSION AGAINST THIS PLAN, NOT TEST ROT. Measured 2026-08-11 on `feat/mobile-upload-draft-recovery` with every uncommitted change stashed: `buildNav.test.js` (1), `mobileNav.test.js` (4), `NavBar.test.jsx` (3) and `AppShell.render.test.jsx` (4) all fail on the same fact — the writer's mobile bottom bar is now `dashboard, create, messages, profile` where the approved contract is `dashboard, projects, messages, profile`. `mobileNav.test.js` names the rule outright ('keeps Create out of the writer's compact bar, but not out of the app'), and §8.2's four-slot bottom nav is why. The source is `layouts/app-shell/navigation/presets/writerNav.js`, changed by commit 2113dc9 'Remove Projects option from desktop sidebar rail and update mobile bottom bar' — the preset feeds BOTH platforms, so a rail edit silently rewrote the phone's bottom bar. **Attribution measured, not inferred:** checking out that one file at 2113dc9~1 and re-running the four suites leaves exactly 2 failures — 'gives a producer the rail, the topbar and the burger' and 'shows the message count on the rail and the mobile bar', the two long-documented pre-existing `AppShell.render.test.jsx` failures, by name. So 10 of the 11 are that single commit (`AppShell`'s 4 = those 2 plus 2 new), and the 11th is `features/producer-workspace/WriterRosterPage.render.test.jsx` > 'explains itself on the profile action, and offers both routes out', which is unrelated and unattributed. Reverting the preset would undo a deliberate desktop decision, so this is a product call, not a test fix: either the mobile contract changes and the four suites are updated with it, or the preset gains a mobile-specific slot. Ask before touching either."
active_files: []
known_blockers: []
last_updated: "2026-08-11"
updated_by: "Claude Phase 3 thirteenth-session completion (Version history, D19, DEF-17/18)"
```
...
**Latest Phase 3 evidence (2026-08-12, fifteenth session):** Create-project media now shares the real byte-progress uploader with `/upload`, moves to a visible Media panel after the project record is saved, and retries only failed files against the retained script id. The focused 153-test seam, broader 457-test regression set, production build/53-route prerender, and 15-state-width CDP matrix pass. Phase 3 remains open for D9 cancel/preflight work and the recorded product decisions; see the newest §19.3 entry.
