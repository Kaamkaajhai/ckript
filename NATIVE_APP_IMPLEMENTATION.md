...
latest_completed_work_item: "Native buyer checkout at /script/:id/pay (2026-08-20, sixth Phase 4 session, D30)."
current_phase: 4
current_work_item: "COMPLETE — Claude — 2026-08-20. D30 made the buyer's checkout native and gave both platforms one definition of it. New: `pages/script-detail/checkout.js`, `pages/script-detail/useProjectCheckout.js`, `mobile/screens/projects/checkout/` (`ProjectCheckoutMobile.{jsx,css}`, `checkoutModel.js`, `checkoutChrome.js`), `mobile/dev/CheckoutHarness.jsx` and `/__mobile-checkout`. Changed: `ScriptPaymentPage.jsx` (659 -> 384 lines, presentation only), the manifest entry `project-payment` promoted from migration fallback to SCREEN, MobileRoutes, the prefix registry (`ckm-checkout`) and the two route tests."
...
next_action: "BEGIN D31: the PUBLIC shared project at `/share/project/:id` — the last unticked bullet of Phase 4 apart from the reader/preview one. It is the first SHARED_PUBLIC_SCREEN of this family: no account, no capabilities object, no purchase controls, and a payload from a different controller than the authenticated detail route, so the first thing to establish is what that payload actually contains versus what `getViewerCapabilities` assumes when `user` is null (`pages/script-detail/scriptDetailModel.js` is written for a signed-in viewer and the public screen must not fabricate an entitlement from its defaults). Read `server/controllers/scriptController.js` `getPublicSharedScript` (or whichever handler `/share/project/:id` resolves to) BEFORE writing any screen, and check it against DEF-25 — the authenticated route shipped `fountainContent` and a private `fileUrl` past its own gate, and the public payload is where that class of leak costs the most. DEF-27 (`GET /scripts/:id/pdf` serving the complete screenplay to anyone past the marketplace gate) is STILL LIVE, still blocked on the product decision below, and D30 sharpened it: buyers now pay through a native checkout for access that endpoint gives away."
open_follow_ups:
  ...
  - "DEF-27 IS LIVE, RECORDED AND NOT FIXED, AND IT IS THE MOST SERIOUS ITEM ON THIS LIST. `GET /scripts/:id/pdf` (`getScriptPdf`) checks the marketplace business-email gate, draft privacy and sold state — and then serves the COMPLETE stored screenplay without ever asking whether the viewer may read the full script. So any authenticated viewer who clears the marketplace gate can fetch the whole PDF of any published, unsold project without buying it. Not inferred: its sibling `exportScreenplayPdf` serves the SAME screenplay in generated form and refuses through `canAccessScript` (owner/admin/collaborator/buyer), and `getScriptById` withholds the same text (see DEF-25, fixed). The gate is not simply added because the desktop Preview panel points `ScreenplayPdfViewer` at this same URL and limits the visible pages IN THE BROWSER — which is also why the leak exists, since client-side page limiting is presentation, not access. Closing it requires choosing what a preview-entitled viewer gets instead: (1) the real PDF sliced server-side (highest fidelity; needs a new dependency — the server has `pdfkit` and `pdf-parse`, neither of which extracts pages — and invalidates the client's `startPage`/`endPage`), (2) a Ckript-generated preview PDF from `scriptPreviewPageTexts` (no new dependency, loses the writer's own typesetting), or (3) the structured-text fallback the viewer already supports (smallest change, loses PDF fidelity for uploaded projects). ASKED OF THE USER IN THE D28 SESSION ENTRY; no answer recorded yet."
...
updated_by: "Claude D29 native project-detail write actions"
```
