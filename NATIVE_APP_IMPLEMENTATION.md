...
| Account entry (sign in, role chooser, password recovery) | `ckm-auth` (`screens/auth/Auth.css`, `/login` + `/join` + `/forgot-password`) — **allocated and registered 2026-08-26**, D59. One family and one stylesheet for all three, and the reserved `ckm-forgot-password` is **retired unused**: recovery is the same dark entry chrome, the same frame and the same single-column editorial form as sign-in, differing only in which three fields it asks for. Two prefixes would have meant two stylesheets sharing every rule, which is the arrangement §7.2 exists to prevent |
| Sign-up stepper | `ckm-signup` (`screens/auth/SignUp.css`, `/signup?as=<role>`) — **allocated and registered 2026-08-26**, D59. **Reallocated from the three reserved prefixes `ckm-writer-onboarding`, `ckm-producer-onboarding` and `ckm-industry-onboarding`, none of which any file ever used**, for the reason `ckm-checkout` replaced `ckm-project-payment` and `ckm-upload` replaced `ckm-upload-project`: the native flow is *one* stepper screen parameterised by role, so three prefixes would have been three names for one stylesheet and no rule would have had an answerable owner. The role is a data attribute on the host, not a prefix. Its own family rather than part of `ckm-auth` — same split as `ckm-editor` vs `ckm-create-project`: a `flow`-shell multi-panel surface with a step rail, a docked keyboard-aware footer and per-step panels shares nothing but tokens with a single-screen sign-in form |
...
active_files:
  ...
  - "client/src/mobile/screens/auth/Auth.css"
  - "client/src/mobile/screens/auth/SignUp.css"
