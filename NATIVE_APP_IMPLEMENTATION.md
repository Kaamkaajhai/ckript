...
latest_completed_work_item: "Native own-profile workspace and shared identity/professional editor (2026-08-21, third Phase 5 session, D36)."
current_phase: 5
current_work_item: "COMPLETE — Codex — 2026-08-21. D36: shared identity/professional profile-editor boundary plus the native own-profile overview/editor across `/profile`, own id/username forms, and signed-in share links. Account/security, bank, and guild-proof operations remain on an explicit desktop settings boundary."
...
next_action: "BEGIN D37: extract the live account/security operations from `Profile.jsx` (email verification/change, password change, session list/revocation, and account deletion) into a tested shared boundary; then implement a native `/profile?tab=settings` screen and remove that query exclusion only after every destructive confirmation and session state is covered. Keep banking and guild-proof review separate, and do not revive the dead `PrivacySettings*` components."
open_follow_ups:
...
| 5. Profiles/network/messages | IN PROGRESS | Codex | 2026-08-21 | — | **D33–D36 COMPLETE:** signed-out public, authenticated visitor, and own-profile overview/editor surfaces are native across share, id and canonical username forms. Shared contracts own loading/actions and identity/professional mutation across desktop and mobile. `GET /users/:id` uses explicit user/project allowlists, closing account-secret and screenplay-body leaks. Account/security, banking/guild proof, reader profiles, inbound follow requests, messages, meetings and collaboration remain. |
