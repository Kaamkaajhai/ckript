...
current_phase: 2
current_work_item: "PHASE 2 IN PROGRESS — bullets 1-5 COMPLETE, only bullet 6 (account/settings and global auth/session behaviours) remains before the phase's exit gate. Bullets 4 and 5 were completed on 2026-08-08. Both of that session's product questions were answered by the user before any code was written — bullet 4's tab set approved as-is (Dashboard · Projects · Messages · Profile), and bullet 5 resolved to 'build /offer-holds for real against the existing getMyHolds API; leave /ai-tools as a dashboard alias'. The §4 gate for that family found the plan's premise was wrong and is recorded in the 2026-08-08 entry in §19.3. Bullets 1 (parity audit), 2 (placeholders/real services) and 3 (dashboard destinations) are COMPLETE — see the 2026-08-07 Phase 2 entry in §19.3. The SectionTabs blocker was answered by the user on 2026-08-07 (option B) and implemented the same session: Projects and Reviews are `/dashboard?tab=...` query destinations declared in the writer preset, NOT new routes — a mobile-only /projects would have broken §5.2's one-canonical-URL rule and the manifest coverage contract. Two defects fell out of it and were fixed: resolveActiveTabKey ignored query strings, and NavBar was marking TWO tabs aria-current because NavLink applies its own."
previous_work_item: "Phase 2 bullets 1-3 (dashboard parity audit, data truth, real destinations), 2026-08-07."
last_completed_work_item: "Phase 2 bullets 4 and 5 (2026-08-08). Bullet 4: the writer tab set approved as-is by the user and now enforced in `mobile/navigation/mobileNav.test.js` on labels, order, and Create's absence from the compact bar alongside its presence in the rail and drawer. Bullet 5: `/offer-holds` built as a real industry screen — `mobile/screens/Holds.{jsx,css}` (prefix `ckm-holds`), `mobile/data/holdsModel.js`, `mobile/hooks/useHoldsData.js` — over `GET /scripts/holds`, a shipped backend that had no client at all; and `/ai-tools` pointed at the mobile dashboard because desktop mounts the identical element there. Both manifest entries moved from DESKTOP_MIGRATION_FALLBACK to SCREEN with audience gates."
...
next_action: "Claim §11 Phase 2 **bullet 6** — `Complete account/settings and global auth/session behaviours` — the last bullet before the phase's exit gate. Nothing blocks it. Start with the §4 gate for that family: mobile's `screens/overlays/AccountMenu.jsx` is currently an action sheet whose items are `/contact`, `/terms`, `/privacy` and the profile, plus a `role=alertdialog` logout confirmation focused on Cancel; desktop's equivalent lives in the app-shell drawer. There is no mobile settings SCREEN and no route for one — check `App.jsx` for what desktop actually mounts before assuming a `/settings` route exists, because the `/ai-tools` finding on 2026-08-08 is a standing warning that a route in the plan is not evidence of a page behind it. Global auth/session behaviours to cover: token expiry mid-session, 401 handling, the `/auth/me` sync that `pages/Dashboard.jsx` runs, and logout from every shell mode. THEN, before Phase 3, two follow-ups this session deliberately left: (a) `/offer-holds` is deep-linkable but reachable from NO navigation — the fix is a `holds` entry in `layouts/app-shell/navigation/presets/industryNav.js`'s drawer, and it must land in the SAME change as a desktop holds screen, because that preset feeds the desktop rail and a link that dumps industry users back on their dashboard is worse than no link; (b) the holds WRITE actions (`releaseHold`, `hold/quote`, `hold/create-order`, `hold/verify-payment`) are all live and unbuilt on both platforms — destructive and money-adjacent, so they need confirmation copy and a refund rule decided first. Independent and safe at any point, unchanged from 2026-08-07: migrate `ckm-tabs` (components/SectionTabs) onto `ckm-tabbar`, the last legacy prefix with a caller; and add the dashboard socket refresh (desktop refetches on seven `collab_*` events, mobile still never refreshes after first load). Do NOT re-derive the dashboard audit (§19.3, 2026-08-07) or the holds audit (§19.3, 2026-08-08); both have payload shapes read out of the server controllers."
superseded_next_action_2026_08_07: "Claim §11 Phase 2 bullet 4 (`Replace provisional bottom navigation with approved writer tabs`) and confirm with the user that the compact writer bar is now the approved set — it currently reads Dashboard · Projects · Messages · Profile, since Create gave up its slot to Projects on 2026-08-07. The bars themselves need no work: they have been role-aware and URL-driven from the desktop presets since Phase 1 (§8.2), and query-string destinations now resolve correctly. Then bullet 5, `/ai-tools` and `/offer-holds` as real route-aware screens — NOT started, no mobile code exists, so begin with the §4 gate for that page family (both are currently DESKTOP_MIGRATION_FALLBACK entries in mobileRouteManifest.js). Then bullet 6, account/settings and global auth/session behaviours. Independent and safe to start at any point: (a) migrate `ckm-tabs` (components/SectionTabs) onto `ckm-tabbar` — it is the last legacy prefix with a caller; (b) add the socket refresh, follow-up 5 below — desktop refetches the dashboard on seven `collab_*` events and mobile still never refreshes after first load. Do NOT re-derive the dashboard audit; it is complete in §19.3, with the payload shapes read out of server/controllers/dashboardController.js."
active_files:
  ...
  - "client/src/mobile/screens/Holds.css"
  - "client/src/mobile/data/holdsModel.js"
  - "client/src/mobile/hooks/useHoldsData.js"
  - "client/src/mobile/theme/cssPrefixRegistry.js"
known_blockers: []
last_updated: "2026-08-08"
updated_by: "Claude Phase 2 bullets 4+5 session (/offer-holds, /ai-tools)"
```
