...
latest_completed_work_item: "Native reader route and server-enforced screenplay access (2026-08-21, eighth Phase 4 session, D32)."
current_phase: 5
current_work_item: "COMPLETE — Codex — 2026-08-21. D32 promoted `/reader/script/:id` without adding a presentation fork, added `projectReaderSource.js` as the pure preview/full source boundary, and added `server/utils/scriptReadAccess.js` as the single full-content authorization predicate. `getScriptPdf` now refuses preview-only viewers before fetching the stored file, closing DEF-27."
...
next_action: "BEGIN D33: Phase 5 profile family gate. Audit own/visitor/public profile route forms (`/profile/:id?`, `/reader/profile/:id?`, `/share/profile/:id`, and canonical username paths), their role shells, edit/completion/follow states, and `getProfileCanonicalPath` before choosing the first vertical slice. Reuse profile data/actions across presentations; do not port the existing 74 kB desktop stylesheet or create separate writer/reader definitions of the same relationship."
open_follow_ups:
