...
- [ ] Public profile/share resolver and canonical username paths. *(D34 completes the signed-out `/share/profile/:id` surface and the existing root-username redirect reaches it; authenticated share, own/visitor, and reader variants remain.)*
...
latest_completed_work_item: "Native signed-out public profile and shared loader contract (2026-08-21, first Phase 5 session, D33/D34)."
current_phase: 5
current_work_item: "COMPLETE — Codex — 2026-08-21. D33/D34 completed the Phase 5 profile-family gate and promoted signed-out `/share/profile/:id` through one cancellable loader shared with desktop and a native defensive view model. Authenticated viewers of the same URL stay on the full desktop profile workspace until D35 ports its permissions and actions."
...
next_action: "BEGIN D35: extract the authenticated profile read/relationship contract from `Profile.jsx` (`GET /users/:id`, follow/request state, message/block/contact capabilities, canonicalization) and implement the visitor slice for `/profile/:id` plus authenticated `/share/profile/:id`. Keep own-profile editing/settings and reader-profile collections separate until their mutations are extracted; reuse `profilePolicy.js` and do not copy the 74 kB desktop stylesheet."
open_follow_ups:
